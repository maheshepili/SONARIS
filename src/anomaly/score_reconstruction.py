...
def score_image(
    ...
    model: SonarAutoencoder | None = None,
    ...
    if model is None or patch_size is None:
    ...
    if not scores:
        raise ValueError(
            f"Image is smaller than the anomaly patch size: {image_path}"
        )
    ...
    result: dict[str, float | bool | str] = {
        ...
        "max_reconstruction_error": round(max_error, 8),
    ...
    if threshold is not None:
        ...
        result["anomalous"] = bool(
            max_error > threshold
        )
        result["excess_over_threshold"] = round(
            max_error - threshold,
            8,
        )
...
def main() -> None:
    ...
    if not args.image.exists() or not args.checkpoint.exists():
        raise SystemExit(
            "Image or anomaly checkpoint does not exist."
        )
